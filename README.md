...
RELAY_PORT='443' \
RELAY_PUBLIC_HOST='RELAY_IP_OR_DOMAIN' \
ROUTE_NAME='US Exit' \
AUTO_YES=1 \
...
若 Relay 同时有 IPv4/IPv6，或某一地址未在上游防火墙开放入口端口，请用
`RELAY_PUBLIC_HOST` 明确指定客户端可达的 IPv4、IPv6 或域名。该值会保存到
线路状态，后续 `--list`、`--qr` 和 `--sub` 会继续使用。
